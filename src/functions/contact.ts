...
const getContactEnquries = async () => {
  try {
    const contactRef = collection(db, "contact");
    const q = query(contactRef, limit(1));
    const querySnapshot = await getDocs(q);
    if (querySnapshot.empty) {
      return null;
    }

    console.log("Contact enquiries:", querySnapshot.docs[0].data().message)
    return querySnapshot.docs[0].data();
  } catch (error) {
    console.error("Error getting contact enquiries:", error);
    return null;
  }
};

export { addContactEnqry, getContactEnquries, sendEmail };
